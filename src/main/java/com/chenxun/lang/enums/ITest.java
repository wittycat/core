...
public class ITest {
	...
	public void enumConstructor(){
		for (ConstructorState state :ConstructorState.values()) {
			System.out.println(state.getDesc());
			System.out.println(state.ordinal());
		}
		
		System.out.println(ConstructorState.END.name());//获取枚举常量名称
		
		ConstructorState constructor = ConstructorState.getConstructor(0);
